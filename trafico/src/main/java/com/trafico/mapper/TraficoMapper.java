...
public interface TraficoMapper {
	...
	@Select("SELECT t.canal_venta_agrup as canalVentaAgrup, "+
			...
			"	WHERE t1.back='TGESTIONA_APP' AND t1.canal_venta_agrup=t.canal_venta_agrup AND MONTH(t1.FECHA_LLAMADA)=6 AND " + 
			"	t1.FLAG_DEPENDENCIA='PRINCIPAL' AND t1.GESTION='CE' " + 
			"	) AS totalVend, " +
			"(SELECT COUNT(DISTINCT t2.COD_VENDEDOR_ATIS) FROM trafico t2 " + 
			"	WHERE t2.back='TGESTIONA_CROSS_WEB' AND t2.canal_venta_agrup=t.canal_venta_agrup AND MONTH(t2.FECHA_LLAMADA)=6 AND " + 
	...
	@Select(" SELECT canal_venta_agrup AS canalVentaAgrup," + 
			...
			"	WHERE t3.back!='TGESTIONA_CROSS_WEB' AND t3.back!='TGESTIONA_APP' AND t3.canal_venta_agrup=t.canal_venta_agrup AND MONTH(t3.FECHA_LLAMADA)=6 AND " + 
	...
	@Select("SELECT " + 
			...
			" AND MONTH(FECHA_LLAMADA)=6 " + 
			"AND ESTADO_GEST_1!='' ")
	List<ConvertibilidadRegistro> convertibilidadRegistro();
